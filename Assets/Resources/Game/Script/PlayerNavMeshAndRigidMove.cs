...
public class PlayerNavMeshAndRigidMove : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        ...
        //agent.Move(Vector3.up * jumpPower);
        // ボタン非押下で勝手に速度減衰
        //
        //charaCon.Move(moveVector * Time.deltaTime);  //CharacterControllerの付いているこのオブジェクトを移動させる処理

        if (playerColliderCheck.GetCollisionEnterExit() == true)
        {
            agent.enabled = true;
